...
export class StaffpaymentComponent implements OnInit {
    ...
    //         currency: 'NGN',
    //         transaction_ref: this.options.ref.toString(),
    //         callback_url: 'http://squadco.com',
    //         // payment_channels: ['card'],
    //     };

    //     this.onlinePaymentService.initiatePayment(payload)
    //         .then(response => {
    ...
    paymentDone(ref: any) {
        ...
        const ord: OrderedMeal[] = [];


        this.onlinePaymentService.postOnlinePayment(pymt, payment, ord, selectedPaymentItems).subscribe(
            (result: any) => {
    ...
    paymentCancel() {
        ...
        this.options.ref = this.generateTransactionRef();
        //console.log('payment failed');
        // this.options.ref = ref;
        //console.log('Payment cancelled');
    }
    setRandomPaymentRef() {
        this.reference = this.generateTransactionRef();
    ...
    //         transaction_ref: this.options.ref.toString(),
    //         callback_url: 'https://cafeteria.evercare.ng'
    //     };


    //     try {
    //         this.onlinePaymentService.initiatePayment(payload).then(response => {
    //             console.log('Payment initiated successfully:', response);
    //             if (response.message === "Success") {
    //                 // const paymentUrl = `https://pay.squadco.com/${payload.transaction_ref}`;

    //                 const paymentUrl = response.data.checkout_url;
    //                 response.data = new squad({
    //                     onClose: () => {
    ...
    //                     // checkout_url: 'https://api-d.squadco.com/transaction/initiate',
    //                     key: "pk_10018e38023e48ac3b29bb286263e659fa8318f2",
    ...
    //                 // fetch(paymentUrl, {
    //                 //     method: 'GET',
    //                 //     mode: 'no-cors' // Ensures a CORS request without response blocking
    //                 // }).then(() => {
    //                 //     console.log('URL loaded in the background:', paymentUrl);
    //                 // }).catch(error => {
    //                 //     console.error('Error loading URL:', error);
    //                 // });


    //                 // const dialogRef = this.dialog.open(SquadPaymentDialogComponent, {
    //                 //     width: '80%',
    ...
    //         // Handle the response or navigate to another page as needed
    //     } catch (error) {
    ...
    SquadPay() {
        ...
        const squadInstance = new squad({
            ...
            email: this.loggedInUser.userName,
            currency_code: "NGN",
            customer_name: `${this.loggedInUser.firstName} ${this.loggedInUser.lastName}`,
            // initiate_type: "inline",
            pass_charge: true,
            // metadata: {
            ...
            transaction_ref: this.options.ref.toString(),
            // callback_url: "localhost:4200/staffpayment"
        });
        squadInstance.setup();
        squadInstance.open();
    ...
    verifyTransaction() {
        ...
        this.onlinePaymentService.verifysquadTransaction(this.options.ref.toString())
            .subscribe(
                response => {
                    this.transactionDetails = response.data;
                    this.errorMessage = ' ';
                    if (this.transactionDetails.transaction_status === "success") {
                        // const selectedPaymentItems: Payment = this.transactionDetails.meta.selectedPaymentItems;
                        this.onlinePaymentService.postOnlinePayment(pymt, payment, ord, selectedPaymentItems).subscribe(
                            (result: any) => {
                                ...
                                // // Call the updatePayment method for each selected payment item
                                // const updateRequests = this.transactionDetails.meta.selectedPaymentItems.map((paymentItem: Payment) => {
                                //     paymentItem.opaymentid = result.id;
                                //     paymentItem.timepaid = new Date();
                                //     paymentItem.paid = true;
                                //     return this.paymentservice.updatePayment(paymentItem);
                ...
                error => {
                    this.errorMessage = 'Failed to verify transaction.';
    ...
    ngOnInit(): void {
        ...
        this.options.metadata = {
            ...
            voucher_id: 0, // Will be updated when calculating total
            payment_type: 'voucher',
            custom_fields: [
